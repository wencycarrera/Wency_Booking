...
<div class="min-h-screen bg-gradient-to-tr from-purple-200 via-pink-100 to-purple-300 py-12 px-4">
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-10 border border-pink-200">
        ...
        <h1 class="text-3xl font-bold text-center text-purple-700 mb-8">Create Booking</h1>
        ...
        <form method="POST" action="{{ route('bookings.store') }}" class="space-y-6">
            ...
            <div class="mt-4 text-center text-pink-600 font-semibold text-lg" id="selected-slot">
                {{ old('booking_date') ? 'Selected: ' . \Carbon\Carbon::parse(old('booking_date'))->format('F j, Y \a\t g:i A') : 'No slot selected.' }}
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const booked = @json($bookedDates); // [{ booking_date: '2025-07-20 14:00:00', title: 'Consultation' }]
    ...
    // Extract full datetime ISO format: "YYYY-MM-DDTHH:MM"
    ...
    // Mark as fully booked if 1 slot per day (or change to 24 if hourly slots allowed)
    const fullDates = Object.entries(fullyBookedDates)
        .filter(([_, count]) => count >= 1) // change to >= 24 if you allow 24/hour slots
        .map(([day]) => day);

    // Prepare events (bookings and background red dates)
